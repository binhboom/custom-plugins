...
if(!class_exists("LocoAutomaticTranslateAddonProBase")) {
	class LocoAutomaticTranslateAddonProBase {
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_automatic-translator-addon-for-loco-translate_fupc', function(){
		...
		final function _CheckWPPlugin( $purchase_key, &$error = "", &$responseObj = null ) {
			...
			$responseObj->license_title = 'Premium License';
			$responseObj->license_key   = 'E2EB9EF2-BC348ED2-39B4AD59-974C6F51';
			$responseObj->is_valid      = true;
			$responseObj->expire_date   = 'no expiry';
			$responseObj->support_end   = date('M d, Y', strtotime('+1 years'));
			$responseObj->next_request  = true;
