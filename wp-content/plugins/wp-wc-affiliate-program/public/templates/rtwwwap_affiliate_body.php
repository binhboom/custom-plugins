...
if( !isset( $_GET[ 'rtwwwap_tab' ] ) || ( isset( $_GET[ 'rtwwwap_tab' ] ) && $_GET[ 'rtwwwap_tab' ] == 'overview' ) ){
	...
	// update code
	$rtwwwap_total_referrals 	= $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(`id`) as total_referrals FROM ".$wpdb->prefix."rtwwwap_referrals WHERE `aff_id`=%d AND `type`!=%d", $rtwwwap_user_id, 3 ) );
	// ends
	...
	// if( $rtwwwap_total_referrals ){
		...
		// for update
		$rtwwwap_html1 .= 				'<th>';
		$rtwwwap_html1 .= 					sprintf( '%s', esc_html__( 'Commission From', 'rtwwwap-wp-wc-affiliate-program' ) );
		$rtwwwap_html1 .= 				'</th>';
		// end here
		...
		if(!empty($rtwwwap_user_all_referrals)){
			foreach( $rtwwwap_user_all_referrals as $rtwwwap_user_ref_key => $rtwwwap_user_ref_value ){
				$rtwwwap_full_name = "";
				$rtwwwap_first_name = "";
				$rtwwwap_last_name = "";
				if($rtwwwap_user_ref_value[ 'type' ] == 0 || $rtwwwap_user_ref_value[ 'type' ] == 4){
					$rtwwwap_get_order_id = $rtwwwap_user_ref_value['order_id'];
					$rtwwwap_order 		= wc_get_order( $rtwwwap_get_order_id );
					if( $rtwwwap_order ){
						$rtwwwap_first_name = $rtwwwap_order->get_billing_first_name();
						$rtwwwap_last_name = $rtwwwap_order->get_billing_last_name();
					}
					
					$rtwwwap_full_name = $rtwwwap_first_name.' '.$rtwwwap_last_name;
				}
				if( $rtwwwap_user_ref_value[ 'type' ] != 3 ) {
					$rtwwwap_html1 .= 		'<tr>';
					$rtwwwap_html1 .= 			'<td>';
					if( $rtwwwap_user_ref_value[ 'type' ] == 1 ){
						$rtwwwap_html1 .=				sprintf( '%s', esc_html__( 'Signup Bonus', 'rtwwwap-wp-wc-affiliate-program' ) );
						// $rtwwwap_referrer_id = $rtwwwap_user_ref_value[ 'signed_up_id' ];
						$rtwwwap_aff_obj_data = get_user_by('ID',$rtwwwap_user_ref_value[ 'signed_up_id' ]);
						if($rtwwwap_aff_obj_data){
							$rtwwwap_full_name = $rtwwwap_aff_obj_data->user_login;
						}	
					}
					...
					// Custom code to update plugin
					elseif( $rtwwwap_user_ref_value[ 'type' ] == 7 ){
						$rtwwwap_html1 .=				sprintf( '%s', esc_html__( 'CPC Bonus', 'rtwwwap-wp-wc-affiliate-program' ) );
					}
					// Custom code ends here
					elseif( $rtwwwap_user_ref_value[ 'type' ] == 15 ){
						$rtwwwap_html1 .=				sprintf( '%s', esc_html__( 'Rank Bonus', 'rtwwwap-wp-wc-affiliate-program' ) );
					}
					elseif( $rtwwwap_user_ref_value[ 'type' ] == 10 ){
						$rtwwwap_html1 .=				sprintf( '%s', esc_html__( 'CF7 Signup Bonus', 'rtwwwap-wp-wc-affiliate-program' ) );
					}
					$rtwwwap_html1 .= 			'</td>';
					$rtwwwap_html1 .= 			'<td>';
					...
					$rtwwwap_html1 .= 				esc_html( $rtwwwap_full_name );
					$rtwwwap_html1 .= 			'</td >';
					// ends here
	...
	// }
		...
		if(in_array('addon_for_affiliate_referral_form/affiliate_referral_form_addon.php', apply_filters('active_plugins', get_option('active_plugins') ) ) ){
				$rtwwwap_html2 = '';
				$rtwwwap_html1 .= apply_filters('rtwwwap_show_forms_aff', $rtwwwap_html2 = "");
		}
		//coupons
			...
			if( !empty( $rtwwwap_levels_settings ) )
			{
				...
				foreach( $rtwwwap_levels_settings as $rtwwwap_levels_key => $rtwwwap_levels_val )
				{
					...
					elseif( $rtwwwap_levels_val[ 'level_criteria_type' ] == 2 )
					{
						...
						$rtwwwap_html1 .= sprintf( '<span> %s',esc_html__( 'Total sale amount ', 'rtwwwap-wp-wc-affiliate-program' ).$rtwwwap_currency_sym.number_format( $rtwwwap_levels_val[ 'level_criteria_val' ],$rtwwwap_decimal_place,$rtwwwap_decimal_separator, $rtwwwap_thousand_separator) .'</span>');
					}
					$rtwwwap_html1 .= 			'</td>';
					$rtwwwap_html1 .= 		'</tr>';
				}
		...
		if(isset($_POST['rtwwwap_profile_save']) && !empty($_POST['rtwwwap_profile_save'])){
			...
			if(!empty($rtwwwap_reg_custom_fields)){
				foreach($rtwwwap_reg_custom_fields as $rtwwwap_key => $rtwwwap_value){
					$rtwwwap_custom_fields = sanitize_text_field($_POST[$rtwwwap_value['custom-input-id']]);
