...
class Rtwwwap_Wp_Wc_Affiliate_Program_Admin {
	...
	public function __construct( $rtwwwap_plugin_name, $rtwwwap_version ) {
		...
		// update 8.3.0 code starts
		$rtwwwap_payout_csv = isset($_GET['payout_csv_download']) ? sanitize_text_field($_GET['payout_csv_download']):'';

		$rtwwwap_referral_csv = isset($_GET['referral_csv_download']) ? sanitize_text_field($_GET['referral_csv_download']):'';

		$rtwwwap_affiliate_csv = isset($_GET['affiliate_csv_download']) ? sanitize_text_field($_GET['affiliate_csv_download']):'';
		if(!empty($rtwwwap_payout_csv))
		{
			add_action('init', array($this, 'rtwwwap_download_payout_csv'));
		}

		if(!empty($rtwwwap_referral_csv))
		{
			add_action('init', array($this, 'rtwwwap_download_referral_csv'));
		}

		if(!empty($rtwwwap_affiliate_csv))
		{
			add_action('init', array($this, 'rtwwwap_download_affiliate_csv'));
		}
		// code ends here
	...
	*/
	...
	// update 8.3.0 code starts
	public function rtwwwap_download_payout_csv(){

		require_once( WP_PLUGIN_DIR."/wp-wc-affiliate-program/third_party/csv-9.8.0/autoload.php");
		$rtwwwap_file_name 	=  sanitize_text_field($_GET[ 'payout_csv_download' ]);
	...
	public function rtwwwap_download_referral_csv(){
		
		require_once( WP_PLUGIN_DIR."/wp-wc-affiliate-program/third_party/csv-9.8.0/autoload.php");
		$rtwwwap_file_name 	=  sanitize_text_field($_GET[ 'referral_csv_download' ]);
		if(empty($rtwwwap_file_name )){
			wp_die(esc_html__('Invalid file name.','rtwwwap-wp-wc-affiliate-program'));
		}
	...
	public function rtwwwap_download_affiliate_csv(){
	...
	*/
	function rtwwwap_save_custom_user_profile_fields_add( $rtwwwap_user_id ){
	    ...
	    if( $rtwwwap_value == 1 && isset( $_POST[ 'rtwwwap_affiliate_level' ] ) ){
		    $rtwwwap_aff_level = sanitize_text_field($_POST[ 'rtwwwap_affiliate_level' ]);
	...
	*/
	function rtwwwap_save_custom_user_profile_fields_edit( $rtwwwap_user_id ) {
		...
		$audit_status = isset($_POST['audit_status'])? sanitize_text_field($_POST['audit_status']): "";
		$aff_qualification = isset($_POST['aff_qualification'])? sanitize_text_field($_POST['aff_qualification']): "";
	...
	*/
	...
	function rtwwwap_approve_callback()
	{
		...
		if ( $rtwwwap_check_ajax ) {
			global $wpdb;
			$rtwwwap_reff_ids 		= isset($_POST[ 'rtwwwap_referral_ids' ]) ? sanitize_text_field($_POST[ 'rtwwwap_referral_ids' ]): "";
			$rtwwwap_reff_ids = json_decode(stripslashes($rtwwwap_reff_ids),true);
			$rtwwwap_approved_ids 	= array();
			if( RTWWWAP_IS_WOO == 1 ){
				$rtwwwap_currency = get_woocommerce_currency();
			}
	...
	function rtwwwap_reject_callback()
	{
		...
		if ( $rtwwwap_check_ajax ) {
			...
			$rtwwwap_reff_ids 		= isset($_POST[ 'rtwwwap_referral_ids' ]) ? sanitize_text_field($_POST[ 'rtwwwap_referral_ids' ]):"";
			$rtwwwap_reff_ids = json_decode(stripslashes($rtwwwap_reff_ids),true);
			$rtwwwap_reject_msg	    = isset($_POST['rtwwwap_reject_message']) ? sanitize_text_field($_POST['rtwwwap_reject_message']):"";
	...
	*/
	function rtwwwap_paypal_callback(){
		$rtwwwap_aff_ids = isset( $_POST[ 'rtwwwap_aff_ids' ] ) ? sanitize_text_field($_POST[ 'rtwwwap_aff_ids' ]) : array();
		$rtwwwap_aff_ids = json_decode(stripslashes($rtwwwap_aff_ids),true);
		$rtwwwap_transaction_id = isset( $_POST[ 'rtwwwap_transaction_id' ] ) ? sanitize_text_field($_POST[ 'rtwwwap_transaction_id' ]) : '';
	...
	function rtwwwap_aff_level_delete_callback()
	{
		...
		if ( $rtwwwap_check_ajax ) {
			$rtwwwap_level_id 	= isset($_POST[ 'rtwwwap_level_id' ]) ? sanitize_text_field($_POST[ 'rtwwwap_level_id' ]):"";
			$rtwwwap_levels_settings = get_option( 'rtwwwap_levels_settings_opt' );
			if( !empty( $rtwwwap_levels_settings ) ){
	...
	function rtwwwap_referral_delete_callback()
	{
		...
		if ( $rtwwwap_check_ajax ) {
			$rtwwwap_referral_id 		= isset($_POST[ 'rtwwwap_referral_id' ]) ? sanitize_text_field($_POST[ 'rtwwwap_referral_id' ]):"";
	...
	*/
	...
	function rtwwwap_payment_pay_callback()
	{
		...
		$rtwwwap_amount = isset( $_POST[ 'rtwwwap_amount' ] ) ? sanitize_text_field($_POST[ 'rtwwwap_amount' ]) : '';
		$rtwwwap_transaction_id = isset( $_POST[ 'rtwwwap_transaction_id' ] ) ? sanitize_text_field($_POST[ 'rtwwwap_transaction_id' ]) : '';
	...
	*/
	function rtwwwap_update_level_order_callback(){
		$rtwwwap_new_order = isset( $_POST[ 'rtwwwap_new_order' ] ) ? sanitize_text_field($_POST[ 'rtwwwap_new_order' ]) : array();
		$rtwwwap_levels_settings = get_option( 'rtwwwap_levels_settings_opt' );
	...
	function rtwwwap_get_mlm_chain_callback(){
		...
		echo json_encode( array( 'rtwwwap_tree_html' => $rtwwwap_final_html, 'rtwwwap_improper_chain' => $rtwwwap_improper_chain ) ); wp_die();
	}
	...
	*/
	...
	function rtwwwap_custom_banner_callback()
	{
		...
		if ( $rtwwwap_check_ajax ) {
			...
			$rtwwwap_image_id = sanitize_text_field($_POST[ 'rtwwwap_image_id' ]);
			$rtwwwap_target_link = sanitize_text_field($_POST[ 'rtwwwap_target_link' ]);
			$rtwwwap_image_dimention_width = sanitize_text_field($_POST[ 'rtwwwap_image_dimention_width' ]);
			$rtwwwap_image_dimention_height = sanitize_text_field($_POST[ 'rtwwwap_image_dimention_height' ]);
	...
	{
		$rtwwwap_aff_id = isset( $_POST['rtwwwap_select_affiliate'] ) ?  sanitize_text_field($_POST['rtwwwap_select_affiliate']) : 0;
	...
	{
		$rtwwwap_noti_title   =	isset($_POST['rtwwwap_not_title'])? sanitize_text_field($_POST['rtwwwap_not_title']) : "" ; 
		$rtwwwap_noti_content = isset($_POST['rtwwwap_no_text'])? sanitize_text_field($_POST['rtwwwap_no_text']) : "" ;
		$rtwwwap_key = isset($_POST['rtwwwap_key'])? sanitize_text_field($_POST['rtwwwap_key']) : "" ;
	...
	function rtwwwap_save_rank_requirement_callback(){
		...
		$rank_already_exist = isset($_POST['index'])? sanitize_text_field($_POST['index']):"";

		$date = date('Y-m-d');
		$rtwwwap_field_detail = array(
			'rank_name' => isset($_POST['rankName']) ? sanitize_text_field($_POST['rankName']):"",
			'rank_desc' => isset($_POST['rankDesc']) ? sanitize_text_field($_POST['rankDesc']):"",
			'rank_priority' => isset($_POST['rankPriority']) ? sanitize_text_field($_POST['rankPriority']):"",
			'date' => $date,
			'rank_commission' => isset($_POST['rankCommission']) ? sanitize_text_field($_POST['rankCommission']):"",
	...
	function rtwwwap_delete_rank_callback(){
		$current_rank_id = isset($_POST['index'])?sanitize_text_field($_POST['index']):"";
		$get_all_rank = get_option('rtwwwap_rank_details');
		$user = get_users();
		if(!empty($user)){
	...
	function rtwwwap_save_customize_email_callback(){
		...
		$customize_subject = isset($_POST['customize_subject'])? sanitize_text_field($_POST['customize_subject']): "";
		$customize_content = isset($_POST['customize_content'])? sanitize_text_field($_POST['customize_content']): "";
	...
	function rtwwwap_edit_customize_email_callback(){
		$email_type = isset($_POST['emailType'])? sanitize_text_field($_POST['emailType']): "";
		$all_emails = get_option('customize_email','null');
	...
	function rtwwwap_activate_email_callback(){
		$email_check = isset($_POST['emailCheck'])? sanitize_text_field($_POST['emailCheck']): "";
		$email_type = isset($_POST['email_type'])? sanitize_text_field($_POST['email_type']): "";
	...
	// update code starts here 8.3.0

	function rtwwwap_generate_csv_payout_callback(){
		global $wpdb;
		$rtwwwap_verification_done = get_option( 'rtwwwap_verification_done', array() );
		...
		$header = ['S.NO.', 'Affiliate ID', 'Affiliate Name', 'Request Date','Amount','Pay Status' ];

		$csv->insertOne($header);

		if(!empty($rtwwwap_withdrawal_all_request)){

			foreach( $rtwwwap_withdrawal_all_request as $rtwwwap_key => $rtwwwap_value ){

				$rtwwwap_aff_info 		= get_userdata( $rtwwwap_value[ 'aff_id' ] );
				$rtwwwap_aff_name 		= ( $rtwwwap_aff_info ) ? $rtwwwap_aff_info->user_login : '';
				$rtwwwap_counter++;
				$csv->insertOne([$rtwwwap_counter, $rtwwwap_value['aff_id'], $rtwwwap_aff_name, $rtwwwap_value['request_date'], $rtwwwap_value['amount'],$rtwwwap_value['pay_status']]);
			}
			...
			echo json_encode( array('status' => false, 'filename' => esc_html__( "There is no data available in this table", 'rtwwwap-wp-wc-affiliate-program' )) );
			wp_die();
		}
	}

	function rtwwwap_generate_csv_affiliate_callback(){
		...
		$rtwwwap_filename 	= "export-labels--".time().".csv";
		$csv = Writer::createFromPath(RTWWWAP_DIR.'assets/csv/'.$rtwwwap_filename, 'w+');
		$header = ['S.NO.','Affiliate ID', 'Affiliate Name', 'Affiliate Email','Parent Id','Parent Name' ];
		$csv->insertOne($header);
		if(!empty($rtwwwap_users))
		{
		  	foreach( $rtwwwap_users as $rtwwwap_key => $rtwwwap_user ){
				...
				// $rtwwwap_parent_info 		= get_userdata( $rtwwwap_user->ID);
				// $rtwwwap_parent_name 		= ( $rtwwwap_parent_info ) ? $rtwwwap_parent_info->user_login : '';
				$rtwwwap_counter++;
				$csv->insertOne([$rtwwwap_counter, $rtwwwap_user->ID , $rtwwwap_user->data->user_login , $rtwwwap_user->data->user_email,$rtwwwap_parent_id, $rtwwwap_parent_name ]);
			}
			echo json_encode( array('status' => true, 'filename' => $rtwwwap_filename) );
			wp_die();
		}
	...
	function rtwwwap_generate_referral_csv_callback(){
		global $wpdb;
		$rtwwwap_all_referrals 	= $wpdb->get_results( "SELECT * FROM ".$wpdb->prefix."rtwwwap_referrals WHERE `type` != 3 ORDER BY `id` DESC", ARRAY_A );
		...
		$rtwwwap_filename 	= "export-labels--".time().".csv";
		$csv = Writer::createFromPath(RTWWWAP_DIR.'assets/csv/'.$rtwwwap_filename, 'w+');
		$header = ['S.NO.','Order ID', 'Affiliate ID', 'Affiliate Name', 'Request Date','Amount','Device' ];

		$csv->insertOne($header);

		if(!empty($rtwwwap_all_referrals)){
			...
			foreach( $rtwwwap_all_referrals as $rtwwwap_key => $rtwwwap_value ){
				...
				if($rtwwwap_value['order_id'] == 0){
					$rtwwwap_order_id = "other commission";
				}
				else{
					$rtwwwap_order_id = $rtwwwap_value['order_id'];
				}
				$csv->insertOne([$rtwwwap_counter, $rtwwwap_order_id, $rtwwwap_value['aff_id'], $rtwwwap_aff_name, $rtwwwap_value['date'], $rtwwwap_value['amount'],$rtwwwap_value['device']]);
			}
